...
public class AuthUIHandler : MonoBehaviour
{
    ...
    void IdTokenChanged(object sender, System.EventArgs eventArgs)
    {
        ...
        {
            senderAuth.CurrentUser.TokenAsync(false).ContinueWith(task => Debug.LogFormat("Token[0:8] = {0}", task.Result.Substring(0, 8)));
        }
    }
    ...
    Task HandleCreateUserAsync(Task<Firebase.Auth.FirebaseUser> authTask, string newDisplayName = null)
    {
        ...
        if (LogTaskCompletion(authTask, "User Creation"))
        {
            ...
            {
                Debug.LogFormat("User Info: {0}  {1}", auth.CurrentUser.Email,auth.CurrentUser.ProviderId);
    ...
    public Task SigninAsync()
    {
        ...
        DisableUI();
        return auth.SignInWithEmailAndPasswordAsync(email, password).ContinueWith(HandleSigninResult);
    }
    ...
    public void SwapAuthFocus()
    {
        ...
        otherAuth = auth;
        auth = swapAuth;
        Debug.LogFormat("Changed auth from {0} to {1}", otherAuth.App.Name, auth.App.Name);
    }
